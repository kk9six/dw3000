...
void loop() {
    ...
    if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) {
        ...
        if (frame_len <= sizeof(rx_buffer)) {
             ...
             * relevant, it is cleared to simplify the validation of the frame.
             */
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0) {
                ...
                // Serial.print("\t");
                // Serial.println(resp_tx_time);

                /* Response TX timestamp is the transmission time we programmed
                 * plus the antenna delay. */
                ...
                if (ret == DWT_SUCCESS) {
                    /* Poll DW IC until TX frame sent event set. See NOTE 6
                     * below. */
